<template>
    <div>
        ...
        <v-row v-if="file" justify="center">
            <v-col cols="9">
                <v-card class="px-0 py-0 mx-auto" :width="backWidth" flat>
                    ...
                    :width="backWidth"
                    class="_background"
                    :src="require(`~/static/images/kesho-${storedType}_03.png`)">
                    <img
                    :width="frontWidth"
                    :height="frontHeight"
                ...
                <v-row class="mx-10">
                    ...
                    <v-icon size="17">mdi-hand-pointing-down</v-icon>
                    </div>
                    <v-btn @click="onAnotherOne" class="mx-auto mt-1" outlined>
                        کلیک کنید
                    </v-btn>
                </v-row>
            </v-col>
        ...
        <v-row>
            <client-only>
                <gallery :num="3" @on-image="file = $event[0]" />
            </client-only>
        </v-row>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            addToStore() {
                ...
                this.$store.commit("SET_TOP" , top)
                !anotherOne ? this.$store.commit("SET_BOTTOM" , top) : ''
            } ,
            onAnotherOne() {
                this.$emit('another-one')
